...
from ska_tmc_common.exceptions import DeviceNameIncorrect
...
class BaseTmcComponentManager(TaskExecutorComponentManager):
    ...
    def update_device_availabiity_for_subscription(self, device_name: str):
        """
    ...
    * Receiving the change events from the component
    ...
    def check_device_responsiveness(self, device_name: str) -> bool:
        ...
        with self.rlock:
            ...
            if not device_information:
                return False
            if device_information.dev_name != device_name:
                self.logger.debug("device name incorrect")
                raise DeviceNameIncorrect(
                    ("Device name is incorrect %s", device_name)
                )
            return not device_information.unresponsive
