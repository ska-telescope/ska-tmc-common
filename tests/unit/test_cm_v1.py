...
def test_start_liveliness_probe_single_device(component_manager_v1):

    component_manager_v1.start_liveliness_probe(
        LivelinessProbeType.SINGLE_DEVICE
    )
    assert component_manager_v1.liveliness_probe_object is not None


def test_update_exception_for_unresponsiveness(component_manager_v1):
    # Test if update_exception_for_unresponsiveness sets the
    # device's exception and does not raise an exception
    exception = "test exception"
    dev_info = component_manager_v1.get_device()
    component_manager_v1.update_exception_for_unresponsiveness(
        dev_info, exception
    )
    assert component_manager_v1.get_device().exception == exception


def test_update_device_info(component_manager_v1):
    ...
    component_manager_v1.update_device_info(device_info)
    assert component_manager_v1.get_device() == device_info


def test_update_responsiveness_info(component_manager_v1):
    # Test if update_ping_info sets the
    # device's ping and does not raise an exception

    dev_name = DUMMY_MONITORED_DEVICE
    component_manager_v1.update_responsiveness_info(dev_name)
    assert not component_manager_v1.get_device().unresponsive


def test_update_device_health_state(component_manager_v1):
    # Test if update_device_health_state updates the
    # device's health state and does not raise an exception
    health_state = HealthState.OK
    component_manager_v1.update_device_health_state(
        DUMMY_MONITORED_DEVICE, health_state
    )
    assert component_manager_v1.get_device().health_state == health_state
    assert (
        component_manager_v1.get_device().last_event_arrived
        == pytest.approx(time.time(), abs=1e-3)
    )
    assert not component_manager_v1.get_device().unresponsive


def test_update_device_state(component_manager_v1):
    # Test if update_device_state updates
    # the device's state and does not raise an exception
    state = tango.DevState.ON
    component_manager_v1.update_device_state(DUMMY_MONITORED_DEVICE, state)
    assert component_manager_v1.get_device().state == state
    assert (
        component_manager_v1.get_device().last_event_arrived
        == pytest.approx(time.time(), abs=1e-3)
    )
...
def test_update_device_obs_state(component_manager_v1):
    # Test if update_device_obs_state updates
    # the device's obs state and does not raise an exception
    obs_state = ObsState.READY
    component_manager_v1.update_device_obs_state(
        DUMMY_MONITORED_DEVICE, obs_state
    )
    assert component_manager_v1.get_device().obs_state == obs_state
    assert (
        component_manager_v1.get_device().last_event_arrived
        == pytest.approx(time.time(), abs=1e-3)
    )
...
def test_command_id_property(component_manager_v1):
    # Test if command id property can be accessed and set correctly
    assert component_manager_v1.command_id == ""
    new_id = f"{time.time()}-TempID"
    component_manager_v1.command_id = new_id
    assert component_manager_v1.command_id == new_id
